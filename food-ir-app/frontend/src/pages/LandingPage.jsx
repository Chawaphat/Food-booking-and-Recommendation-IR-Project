  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        setRandomRecipes(
          Array.isArray(randoms) ? randoms : randoms?.data || [],
        );
      } catch (e) {
      ...
      try {
        ...
        const allPreviews = await Promise.all(
          folderList.map(async (folder) => {
            try {
              ...
              const recipes = Array.isArray(recipesRes)
                ? recipesRes
                : recipesRes?.data || [];
              ...
              console.error(
                `Failed to load preview for folder ${folder.id}`,
                e,
              );
              return { folder, recipes: [] };
            }
        ...
        const validPreviews = allPreviews.filter(
          (p) => Array.isArray(p.recipes) && p.recipes.length > 0,
        );
        setFolderPreviews(validPreviews);
      } catch (e) {
      ...
      try {
        ...
        const bmsRecipes = Array.isArray(bmsRecipesRes)
          ? bmsRecipesRes
          : bmsRecipesRes?.data || [];
      ...
      <div className="bg-white px-6 py-12 md:py-20 rounded-b-[3rem] shadow-soft mb-12">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-lg text-gray-500 mb-10 max-w-xl mx-auto">
            Discover your next favorite dish and save it for later. Search
            thousands of recipes instantly.
          </p>
              ...
              {folderPreviews.map(({ folder, recipes }) => {
                ...
                return (
                  <div
                    ...
                    className="min-w-[280px] sm:min-w-[320px] max-w-[350px] snap-start flex-none group cursor-pointer bg-white rounded-3xl overflow-hidden shadow-soft hover:shadow-soft-lg transition-all duration-300 transform hover:-translate-y-1"
                  >
                    {/* Image mosaic */}
                    <div className="relative h-44 bg-gray-100 overflow-hidden">
                      {images.length === 0 ? (
                        ...
                        <div
                          className={`grid h-full gap-0.5 ${images.length >= 4 ? "grid-cols-2 grid-rows-2" : "grid-cols-2"}`}
                        >
                          {images
                            .slice(0, images.length >= 4 ? 4 : 2)
                            .map((recipe, idx) => (
                              <img
                                key={idx}
                                src={
